...
public class Example004_HumanReadableFile {
    public static void main(String[] args) {
        ...
        Path outDir = Paths.get("").resolve("data").toAbsolutePath(); // outDir ist ein Verzeichnis
        ...
        // Der scanner geht über alle Tabs, Umbrüche, Abstände hinweg und sucht den nächsten Boolean.
        // Daher ergibt das keine Probleme, dass wir Zeilenumbrüche geschrieben haben beim Array.
        // read data
        int height = -1;
        int width = -1;
        ...
        try(Scanner scanner = new Scanner(outFile.toFile())) {
            ...
            for(int row = 0; row < height; row += 1) {
                for(int col = 0; col < width; col += 1) {
                    readField[row][col] = scanner.nextBoolean();
                }
            }            
        } catch (IOException ie) { 
            ie.printStackTrace();
